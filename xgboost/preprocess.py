...
def cut():
    ...
    def sample(data,targetfile):
        ...
        return
    sourcePath = 'train/data_init'
    targetPath = 'train/data_init/data'
    id = 1
    import os
...
'''
encode method
input:dataset is a dataframe dataset
output:dataset's feature vector
describe:
关于数据格式可以参考data_init/data下的数据集
工作完成后请给出输出维度，也就是特征向量的维度
如果有更改目录结构，请说明
'''
def encode(dataset):
    vector = []
...
label method
input:dataset is a dataframe dataset
output:params are the best params of the dataset
describe:none
'''
...
# cut()
import os
cns = os.listdir('accucn/')
cnans = []
for file in cns:
    f = open('accucn/' + file,'r')
    cnans.append(f.readline())
    f.close()
bs = os.listdir('accucn/')
bsans = []
for file in bs:
    f = open('accubs/' + file,'r')
    bsans.append(f.readline())
    f.close()
problemlis = []
for i in range(len(cns)):
    our = float(cnans[i].split(',')[0])
    it = float(bsans[i].split(',')[0])
    if our > it:
        # print(cnans[i])
        # print(bsans[i])
        # print(cns[i])
        if our - it >= 0:
            problemlis.append(int(cns[i].replace('result','').replace('.txt','')))
            print(it - our)
            print(cns[i])
            print('\n\n')
problemlis.sort()
